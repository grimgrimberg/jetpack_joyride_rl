...
from tests.fakes.input import KeyEventType
...
class TestReward:
    ...
    def test_default_reward_without_ocr(self, default_config, fake_capture, fake_input, fake_window):
        ...
        # Survival bonus starts at 1.0+ (scaled by time alive)
        assert reward >= 1.0, f"Expected survival reward >= 1.0, got {reward}"
        env.close()


class TestGameplayOnly:
    """Tests for gameplay-only mode behavior."""

    def test_gameplay_only_skips_auto_navigation(self, default_config, fake_input, fake_window, monkeypatch):
        """Gameplay-only should not auto-navigate menus during step()."""
        from ppsspp_jetpack_rl import JetpackPPSSPPEnv

        # Uniform gray frame -> SAVE_DIALOG state
        frame = np.full((272, 480, 3), 120, dtype=np.uint8)
        capture = FakeCapture(frames=[frame], width=480, height=272)
        ...
        # Avoid blocking in reset wait
        monkeypatch.setattr(env, "_wait_for_gameplay_manual", lambda *args, **kwargs: True)
        env.reset()
        env._has_seen_gameplay = False

        def _fail_nav(*args, **kwargs):
            raise AssertionError("auto-navigation should not run in gameplay_only mode")

        monkeypatch.setattr(env, "_navigate_to_gameplay", _fail_nav)

        env.step(0)
        env.close()

    def test_gameplay_only_releases_action_on_pause(self, default_config, fake_input, fake_window, monkeypatch):
        """Gameplay-only pause should release held action keys."""
        from ppsspp_jetpack_rl import JetpackPPSSPPEnv
        ...
        monkeypatch.setattr(env, "_wait_for_gameplay_manual", lambda *args, **kwargs: True)
        env.reset()
        env._has_seen_gameplay = True
        env._gameplay_steps = 1

        env.step(1)

        assert env._action_is_down is False
        env.close()

    def test_gameplay_only_ends_on_death_after_gameplay(self, default_config, fake_input, fake_window, monkeypatch):
        """Gameplay-only should still end episodes on death screens after gameplay."""
        from ppsspp_jetpack_rl import GameState, JetpackPPSSPPEnv

        env = JetpackPPSSPPEnv(
            default_config,
            capture_backend=FakeCapture(width=480, height=272),
            input_backend=fake_input,
            window_backend=fake_window,
            gameplay_only=True,
        )
        ...
        env._has_seen_gameplay = True
        env._gameplay_steps = 1
        monkeypatch.setattr("ppsspp_jetpack_rl.detect_game_state", lambda *args, **kwargs: GameState.RESULTS)

        _, reward, done, _, _ = env.step(0)

        assert done is True
        # Death penalty is now scaled: -5.0 - (gameplay_steps / 20.0)
        assert reward < 0, f"Expected negative death penalty, got {reward}"
        assert reward >= -15.0, f"Death penalty too harsh: {reward}"
        env.close()
